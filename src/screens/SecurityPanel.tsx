...
// USA TU CLAVE AIza... (La extensión CORS se encargará de que pase)
...
const SecurityPanel: React.FC<Props> = ({ setScreen, onLogout }) => {
  ...
  // --- FUNCIÓN DE NOTIFICACIÓN (DIRECTA - REQUIERE EXTENSIÓN CORS) ---
  const notifyOwner = async (invitation: AccessInvitation) => {
    try {
      ...
      if (userSnap.exists()) {
        ...
        if (token) {
          console.log("🔔 Enviando notificación DIRECTA a:", userData.name);
          
          // SIN PROXIES - DIRECTO A GOOGLE
          // ESTO REQUIERE LA EXTENSIÓN DE CHROME "ALLOW CORS" ACTIVADA
          ...
          if (response.ok) {
            ...
            console.log("⚠️ El dueño no tiene token registrado.");
        }
      }
    } catch (error) {
      console.error("Error FATAL:", error);
      alert("⚠️ Error de Red: Asegúrate de tener la extensión 'Allow CORS' activada en Chrome.");
    }
  };

  // ... (RESTO DEL CÓDIGO IGUAL: handleQrScan, processAccess, etc.)
